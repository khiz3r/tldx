...
type Config struct {
	...
	outputFile string
	silent     bool
	noOutput   bool // suppress all stdout (hits still saved to -o file)
...
//   tldx.example.com   → [no wildcard, return as-is for normal append mode]
//
...
func NewEngine(cfg *Config) *Engine {
	...
	// Single goroutine owns all stdout writes — eliminates interleaved escape codes
	e.printerWg.Add(1)
	go func() {
		defer e.printerWg.Done()
		for line := range e.printCh {
			fmt.Println(line)
		}
	}()
	return e
...
func (e *Engine) printBanner() {
	if e.cfg.silent || e.cfg.noOutput {
		return
	}
	...
	fmt.Fprintf(os.Stderr, " Depth    : %s\n", depth)
	fmt.Fprintf(os.Stderr, " Threads  : %d\n", e.cfg.threads)
	fmt.Fprintf(os.Stderr, " Status   : %s\n", e.cfg.statusRaw)
	fmt.Fprintf(os.Stderr, " TLDs     : %d\n", len(tlds))
	if e.cfg.userAgent != "" {
		fmt.Fprintf(os.Stderr, " UA       : custom\n")
	} else {
		fmt.Fprintf(os.Stderr, " UA       : %s\n", e.cfg.uaType)
	}
	if e.cfg.outputFile != "" {
...
func (e *Engine) Run(inputs []string) {
	...
	if len(wildcardExpanded) > 0 {
		if !e.cfg.silent {
			fmt.Fprintf(os.Stderr, "%s[~] Wildcard mode — probing %d expanded candidates directly%s\n",
				colorGray, len(wildcardExpanded), colorReset)
		}
		hits := e.probeDirectly(wildcardExpanded)
		// Only feed wildcard hits into recursive TLD-append if depth > 1 or unlimited (0)
		if e.cfg.depth == 0 || e.cfg.depth > 1 {
	...
	if len(normalQueue) > 0 {
		...
		for {
			...
			if !e.cfg.silent && !e.cfg.noOutput {
				fmt.Fprintf(os.Stderr, "%s[*] Depth %d — probing %d base(s) × %d TLDs = %d candidates%s\n",
	...
	if !e.cfg.silent && !e.cfg.noOutput {
		fmt.Fprintf(os.Stderr, "\n%s━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━%s\n", colorGray, colorReset)
		fmt.Fprintf(os.Stderr, "%s[✓] Done. %d hit(s) found.%s\n", colorGreen, len(e.results), colorReset)
		if e.cfg.outputFile != "" {
			fmt.Fprintf(os.Stderr, "%s[✓] Results saved to: %s%s\n", colorGreen, e.cfg.outputFile, colorReset)
		}
	}
}
...
func main() {
	...
	silent := flag.Bool("silent", false, "Silent mode — only print URLs (no status/title)")
	noOutput := flag.Bool("no-output", false, "Suppress all stdout hits (useful with -o to write-only)")
